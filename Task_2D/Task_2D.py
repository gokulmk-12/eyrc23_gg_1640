...
def read_csv(csv_name):
    ...
    with open('lat_long.csv', 'r') as points:
...
def write_csv(loc, csv_name):
    ...
    with open(str(csv_name) + '.csv', mode='w', newline='') as coord_file:
        csv_writer = csv.writer(coord_file)
        csv_writer.writerow(['lat', 'lon'])  # Write header row
        csv_writer.writerow([loc[0], loc[1]])  # Write new data
...
def tracker(ar_id, lat_lon):
    ...
    write_csv(coordinate, write_file)
